...
def _build_filter_string(kwargs: Dict[str, Any], tool_args_type: Dict[str, str], fixed_filter: str) -> str:
    """
    Build filter string for Genesis from kwargs
    """
    filter_parts = []
...
class GenesisToolFactory:
    ...
    def create_search_tool(
        ...
        """
        Creates a Genesis search/retrieval tool
        ...
        Args:
            ...
            fixed_filter (str, optional): A fixed Genesis filter condition to apply to all queries.
        ...
        def search_function(*args, **kwargs) -> ToolOutput:
            """
            Dynamically generated function for semantic search Genesis.
            """
            ...
            genesis_retriever = genesis.as_retriever(
            ...
            response = genesis_retriever.retrieve(query)

            if len(response) == 0:
                msg = "Genesis Tool failed to retreive any results for the query."
                return ToolOutput(
                    tool_name=search_function.__name__,
                    content=msg,
                    raw_input={"args": args, "kwargs": kwargs},
    ...
    def create_rag_tool(
        ...
        Args:
            ...
            genesis_prompt_text (str, optional): The prompt text for the Genesis summarizer.
        ...
        def rag_function(*args, **kwargs) -> ToolOutput:
            ...
            genesis_query_engine = genesis.as_query_engine(
                ...
                prompt_text=genesis_prompt_text,
                reranker=reranker,
                rerank_k=rerank_k if rerank_k * self.num_corpora <= 100 else int(100 / self.num_corpora),
                mmr_diversity_bias=mmr_diversity_bias,
                udf_expression=udf_expression,
