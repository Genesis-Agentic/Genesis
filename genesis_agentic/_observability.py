"""
Observability for Genesis Agentic.
"""
import os
import json
...
def eval_fcs() -> None:
    '''
    Evaluate the FCS score for the GenesisQueryEngine._query span.
    '''
    import phoenix as px
    ...
    genesis_spans['top_level_parent_id'] = genesis_spans.apply(
        lambda row: _find_top_level_parent_id(row, all_spans), axis=1
    )
    genesis_spans['score'] = genesis_spans['output.value'].apply(_extract_fcs_value)
